...
class UNetModel_v1preview(nn.Module):
    ...
    def highway_forward(self,x, hs=None, control_hs=None):
        return self.hwm(x, hs=hs, control_hs=control_hs)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        # insert controlnet
        control_hs, control_middle = self.controlnet(x, emb)
        if self.num_classes is not None:
        ...
        for module in self.output_blocks:
            ...
            # apply ControlNet feature injection
            if len(control_hs) > 0:
                ctrl = control_hs.pop()
                h = h + ctrl  # residual merge
            h = module(h, emb)
        h = h.type(x.dtype)
        out = self.out(h)
        return out, cal
...
class UNetModel_newpreview(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        hs = []
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))
        # copy the DDPM input_layers / encoder for controlnet
        control_hs, control_middle = self.controlnet(x, emb)
        if self.num_classes is not None:
            assert y.shape == (x.shape[0],)
            emb = emb + self.label_emb(y)
        ...
        for ind, module in enumerate(self.input_blocks):
            if len(emb.size()) > 2:
                emb = emb.squeeze()
            if ind == 0:
                # here, h is the x_t from q_sample (forward diffusion); emb is the original DDPM Unet output
                h = module(h, emb)
                # apply the conditions to DDPM encoder (use the residual connection)
                h = h + th.cat((uemb[0], uemb[0], uemb[1]),1).detach() # 32 + 32 + 64 in 256 res
            else:
                h = module(h, emb)
            hs.append(h)
        h = self.middle_block(h, emb)
        for module in self.output_blocks:
            # generate through DDPM decoder
            h = th.cat([h, hs.pop()], dim=1)
            h = module(h, emb)
        h = h.type(x.dtype)
        out = self.out(h)
...
class Generic_UNet(SegmentationNetwork):
    ...
    def forward(self, x, hs = None, control_hs=None):
        skips = []
        seg_outputs = []
        anch_outputs = []

        # SSFormer + Fuse the SSFformer and ControlNet
        ...
        # embed the x -> used as the start point for reverse diffusion model
        emb = conv_nd(2, x.size(1), 512, 1).to(device = x.device)(x)

        # dont use DDPM, directly give the segmentation output (seg_outputs[-1])
...
class ControlNet(nn.Module):
    def __init__(self, base_unet: UNetModel_v1preview):
        super().__init__()
        # clone architecture from base
        self.control_blocks = nn.ModuleList([
            deepcopy(block) for block in base_unet.input_blocks # is this the diffusion or segmentation UNet encoder?
        ])
        self.middle_block = deepcopy(base_unet.middle_block)

        # create zero-initialized 1x1 convs DYNAMICLY
        self.zero_convs = nn.ModuleList()

        dummy_x = torch.zeros(1, base_unet.in_channels, base_unet.image_size, base_unet.image_size)
        dummy_emb = torch.zeros(1, base_unet.model_channels * 4)
        with torch.no_grad():
            for block in self.control_blocks:
                dummy_x = block(dummy_x, dummy_emb)
                out_ch = dummy_x.shape[1]
                self.zero_convs.append(zero_module(conv_nd(2, out_ch, out_ch, 1)))

        # middle block's control output
        with torch.no_grad():
            dummy_middle = self.middle_block(dummy_x, dummy_emb)
            middle_ch = dummy_middle.shape[1]
        self.zero_conv_middle = zero_module(conv_nd(2, middle_ch, middle_ch, 1))

    def forward(self, x, t_emb):
        h = x
        control_hs = []
        for block, zero_conv in zip(self.control_blocks, self.zero_convs):
            h = block(h, t_emb)
            control_hs.append(zero_conv(h))  # zero-initialized conv
        h = self.middle_block(h, t_emb)
        control_middle = self.zero_conv_middle(h)
        return control_hs, control_middle